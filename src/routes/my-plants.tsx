...
if(!user)return <main className="mx-auto max-w-2xl p-6 text-center"><Leaf className="mx-auto h-10 w-10 text-primary"/><h1 className="mt-3 text-xl font-bold">Plant Diary</h1><p className="my-3 text-sm text-muted-foreground">Sign in to keep your plant history.</p><Link to="/auth"><Button>Sign in</Button></Link></main>;
return <main className="mx-auto max-w-2xl space-y-3 p-4 pb-28"><h1 className="text-xl font-bold">Plant Diary</h1><p className="text-sm text-muted-foreground">Your saved plants and their growing histories. Private by default; share only when you choose.</p>{q.isLoading&&<p>Loading...</p>}{!q.isLoading&&!q.data?.length&&<Card className="p-6 text-center"><p className="mb-3">No saved plants yet.</p><Link to="/identify"><Button>Identify a plant</Button></Link></Card>}{(q.data||[]).map(p=><Link key={p.id} to="/plant-memory/$id" params={{id:p.id}} className="block"><Card className="p-4"><div className="flex justify-between gap-3"><div><b>{p.common_name||p.scientific_name||"Unknown plant"}</b>{p.common_name&&p.scientific_name&&<p className="text-xs italic text-muted-foreground">{p.scientific_name}</p>}</div><span className="text-xs">{p.is_community_shared?"Shared":"Private"}</span></div><p className="mt-2 text-xs text-muted-foreground">{new Date(p.created_at).toLocaleDateString()}</p></Card></Link>)}</main>}
